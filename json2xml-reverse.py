...
stoptime = '2022-08-16 15:30' # Half an hour before you leave work, so this process can stop and be continued tomorrow.
...
def main():
    ...
    for dir in json_dir.iterdir():
        for photo_index, json_filename in enumerate(dir.iterdir()):
            photo_json = get_json(json_filename)

            # EXIT CONDITIONS
            # TODO Remove testing limt
            # Stop early due to time
            if dff.time_to_stop(stoptime):
                print('-'*40, flush=True)
                print('\nscript ended early for the end of the work day. Please continue tomorrow.', flush=True)
                return
            # Json does not have a photo ID
            if not (photo_id := photo_json.get('id', None)):
                continue

            # Check if json ID is in missing file
            found = any(photo_id in str(file) for file in dir.iterdir())
            if not found:
                with open('jpeg_not_found', 'a') as f:
                    f.write(photo_json + '\n')
...
def find_jpg(json_filename):
    """Find the corresponding jpg file for a photo"""
    """If the jpg is not found, output the photo name to jpg_not_found.txt"""
    try:
        photo_id = str(jpg_filename).split('_')[-2]
        json_filename = f'photo_{photo_id}.json'
        json_filename = find(json_filename, 'json')
